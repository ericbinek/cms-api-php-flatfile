...
final class PersonRouter
{
    ...
    private static function handleCollection(string $method, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'POST') {
            ...
            $body = Person::sanitize(Http::parseBody());
            ...
            Http::json(201, Access::stripFields($role, $created), etag: Person::etagOf($created));
            return;
        }
    ...
    private static function handleItem(string $method, string $id, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'GET') {
            ...
            Http::json(200, Access::stripFields($role, Person::embedRefs($item)), etag: Person::etagOf($item));
            return;
        }
        ...
        if ($method === 'PUT') {
            ...
            Http::json(200, Access::stripFields($role, $updated), etag: Person::etagOf($updated));
            return;
        }
    ...
    private static function parseListOptions(): array
    {
        ...
        $sort = 'dateCreated';
        if (isset($sp['sort'])) {
            if (!in_array($sp['sort'], Person::SORTABLE_FIELDS, true)) {
                $sorted = Person::SORTABLE_FIELDS;
                sort($sorted);
        ...
        foreach ($sp as $key => $value) {
            ...
            if (!in_array($key, Person::SEARCHABLE_FIELDS, true)) {
