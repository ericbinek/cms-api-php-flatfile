...
final class BlogPostingRouter
{
    ...
    private static function handleCollection(string $method, string $requestPath): void
    {
        if ($method === 'GET') {
            ...
            unset($opts['errors']);
            Http::json(200, BlogPosting::findAll($opts));
            return;
        }
        if ($method === 'POST') {
            $body = Http::parseBody();
            $errors = BlogPosting::validate($body);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $created = BlogPosting::create($body);
    ...
    private static function handleItem(string $method, string $id, string $requestPath): void
    {
        ...
        if ($method === 'GET') {
            $item = BlogPosting::findById($id);
            if ($item === null) {
                Http::jsonError(Errors::notFound(BlogPosting::TYPE_NAME, $requestPath));
                return;
            }
            Http::json(200, BlogPosting::embedRefs($item));
            return;
        }

        if ($method === 'PUT') {
            $body = Http::parseBody();
            $errors = BlogPosting::validate($body, partial: true);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $current = BlogPosting::findById($id);
            if ($current === null) {
                Http::jsonError(Errors::notFound(BlogPosting::TYPE_NAME, $requestPath));
                return;
            }
            $ifMatch = $_SERVER['HTTP_IF_MATCH'] ?? null;
            if ($ifMatch !== null && $ifMatch !== '*' && $ifMatch !== BlogPosting::etagOf($current)) {
    ...
    private static function parseListOptions(): array
    {
        ...
        $sort = 'dateCreated';
        if (isset($sp['sort'])) {
            if (!in_array($sp['sort'], BlogPosting::SORTABLE_FIELDS, true)) {
                $sorted = BlogPosting::SORTABLE_FIELDS;
                sort($sorted);
        ...
        foreach ($sp as $key => $value) {
            ...
            if (!in_array($key, BlogPosting::SEARCHABLE_FIELDS, true)) {
