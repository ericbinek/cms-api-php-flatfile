...
final class AudioObjectRouter
{
    public const BASE = '/audio-objects';
    private const ENTITY = 'AudioObject';
    ...
    private static function handleCollection(string $method, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'POST') {
            ...
            $body = AudioObject::sanitize(Http::parseBody());
            ...
            $errors = AudioObject::validate($body);
            if (!empty($errors)) {
    ...
    private static function handleItem(string $method, string $id, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'PUT') {
            ...
            $errors = AudioObject::validate($body, partial: true);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $current = AudioObject::findById($id);
            if ($current === null) {
                Http::jsonError(Errors::notFound(AudioObject::TYPE_NAME, $requestPath));
                return;
            }
            ...
            if ($ifMatch !== null && $ifMatch !== '*' && $ifMatch !== AudioObject::etagOf($current)) {
    ...
    private static function parseListOptions(): array
    {
        ...
        $sort = 'dateCreated';
        if (isset($sp['sort'])) {
            if (!in_array($sp['sort'], AudioObject::SORTABLE_FIELDS, true)) {
                $sorted = AudioObject::SORTABLE_FIELDS;
                sort($sorted);
        ...
        foreach ($sp as $key => $value) {
            ...
            if (!in_array($key, AudioObject::SEARCHABLE_FIELDS, true)) {
