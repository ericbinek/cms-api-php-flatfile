...
final class SiteNavigationElementRouter
{
    public const BASE = '/site-navigation-elements';
    private const ENTITY = 'SiteNavigationElement';
    ...
    private static function handleCollection(string $method, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'GET') {
            ...
            unset($opts['errors']);
            $all = SiteNavigationElement::findAll($opts);
        ...
        if ($method === 'POST') {
            ...
            $body = SiteNavigationElement::sanitize(Http::parseBody());
            ...
            $errors = SiteNavigationElement::validate($body);
            if (!empty($errors)) {
    ...
    private static function handleItem(string $method, string $id, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'GET') {
            ...
            $item = SiteNavigationElement::findById($id);
        ...
        if ($method === 'PUT') {
            ...
            $errors = SiteNavigationElement::validate($body, partial: true);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $current = SiteNavigationElement::findById($id);
            if ($current === null) {
                Http::jsonError(Errors::notFound(SiteNavigationElement::TYPE_NAME, $requestPath));
                return;
            }
            ...
            if ($ifMatch !== null && $ifMatch !== '*' && $ifMatch !== SiteNavigationElement::etagOf($current)) {
    ...
    private static function parseListOptions(): array
    {
        ...
        $sort = 'dateCreated';
        if (isset($sp['sort'])) {
            if (!in_array($sp['sort'], SiteNavigationElement::SORTABLE_FIELDS, true)) {
                $sorted = SiteNavigationElement::SORTABLE_FIELDS;
                sort($sorted);
        ...
        foreach ($sp as $key => $value) {
            ...
            if (!in_array($key, SiteNavigationElement::SEARCHABLE_FIELDS, true)) {
