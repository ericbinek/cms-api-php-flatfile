...
final class CategoryCodeRouter
{
    ...
    private static function handleCollection(string $method, string $requestPath): void
    {
        if ($method === 'GET') {
            ...
            unset($opts['errors']);
            Http::json(200, CategoryCode::findAll($opts));
            return;
        }
        if ($method === 'POST') {
            $body = Http::parseBody();
            $errors = CategoryCode::validate($body);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $created = CategoryCode::create($body);
    ...
    private static function handleItem(string $method, string $id, string $requestPath): void
    {
        ...
        if ($method === 'GET') {
            $item = CategoryCode::findById($id);
            if ($item === null) {
                Http::jsonError(Errors::notFound(CategoryCode::TYPE_NAME, $requestPath));
                return;
            }
            Http::json(200, CategoryCode::embedRefs($item));
            return;
        }

        if ($method === 'PUT') {
            $body = Http::parseBody();
            $errors = CategoryCode::validate($body, partial: true);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $current = CategoryCode::findById($id);
            if ($current === null) {
                Http::jsonError(Errors::notFound(CategoryCode::TYPE_NAME, $requestPath));
                return;
            }
            $ifMatch = $_SERVER['HTTP_IF_MATCH'] ?? null;
            if ($ifMatch !== null && $ifMatch !== '*' && $ifMatch !== CategoryCode::etagOf($current)) {
    ...
    private static function parseListOptions(): array
    {
        ...
        $sort = 'dateCreated';
        if (isset($sp['sort'])) {
            if (!in_array($sp['sort'], CategoryCode::SORTABLE_FIELDS, true)) {
                $sorted = CategoryCode::SORTABLE_FIELDS;
                sort($sorted);
        ...
        foreach ($sp as $key => $value) {
            ...
            if (!in_array($key, CategoryCode::SEARCHABLE_FIELDS, true)) {
