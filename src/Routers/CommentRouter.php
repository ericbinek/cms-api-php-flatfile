...
final class CommentRouter
{
    ...
    private static function handleCollection(string $method, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'POST') {
            ...
            $body = Comment::sanitize(Http::parseBody());
            ...
            $errors = Comment::validate($body);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $created = Comment::create(Access::applyCreateDefaults(self::ENTITY, $body, $principal['accountId']));
            Http::setLocation(self::BASE . '/' . $created['id']);
            Http::json(201, Access::stripFields($role, $created), etag: Comment::etagOf($created));
            return;
        }
    ...
    private static function handleItem(string $method, string $id, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'GET') {
            ...
            if ($item === null || !Access::isVisible($role, self::ENTITY, $item)) {
                Http::jsonError(Errors::notFound(Comment::TYPE_NAME, $requestPath));
                return;
            }
            // The ETag names the stored record's version, not the role- and
            // embedding-shaped body — it must satisfy a later If-Match.
            Http::json(200, Access::stripFields($role, Comment::embedRefs($item)), etag: Comment::etagOf($item));
            return;
        }
        ...
        if ($method === 'PUT') {
            ...
            $errors = Comment::validate($body, partial: true);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $current = Comment::findById($id);
            if ($current === null) {
                Http::jsonError(Errors::notFound(Comment::TYPE_NAME, $requestPath));
                return;
            }
            ...
            if ($ifMatch !== null && $ifMatch !== '*' && $ifMatch !== Comment::etagOf($current)) {
            ...
            $updated = Comment::update($id, $body);
            Http::json(200, Access::stripFields($role, $updated), etag: Comment::etagOf($updated));
            return;
        }
    ...
    private static function parseListOptions(): array
    {
        ...
        $sort = 'dateCreated';
        if (isset($sp['sort'])) {
            if (!in_array($sp['sort'], Comment::SORTABLE_FIELDS, true)) {
                $sorted = Comment::SORTABLE_FIELDS;
                sort($sorted);
        ...
        foreach ($sp as $key => $value) {
            ...
            if (!in_array($key, Comment::SEARCHABLE_FIELDS, true)) {
