...
final class WebSiteRouter
{
    ...
    private static function handleCollection(string $method, string $requestPath): void
    {
        if ($method === 'GET') {
            ...
            unset($opts['errors']);
            Http::json(200, WebSite::findAll($opts));
            return;
        }
        if ($method === 'POST') {
            $body = Http::parseBody();
            $errors = WebSite::validate($body);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $created = WebSite::create($body);
    ...
    private static function handleItem(string $method, string $id, string $requestPath): void
    {
        ...
        if ($method === 'GET') {
            $item = WebSite::findById($id);
            if ($item === null) {
                Http::jsonError(Errors::notFound(WebSite::TYPE_NAME, $requestPath));
                return;
            }
            Http::json(200, WebSite::embedRefs($item));
            return;
        }

        if ($method === 'PUT') {
            $body = Http::parseBody();
            $errors = WebSite::validate($body, partial: true);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $current = WebSite::findById($id);
            if ($current === null) {
                Http::jsonError(Errors::notFound(WebSite::TYPE_NAME, $requestPath));
                return;
            }
            $ifMatch = $_SERVER['HTTP_IF_MATCH'] ?? null;
            if ($ifMatch !== null && $ifMatch !== '*' && $ifMatch !== WebSite::etagOf($current)) {
    ...
    private static function parseListOptions(): array
    {
        ...
        $sort = 'dateCreated';
        if (isset($sp['sort'])) {
            if (!in_array($sp['sort'], WebSite::SORTABLE_FIELDS, true)) {
                $sorted = WebSite::SORTABLE_FIELDS;
                sort($sorted);
        ...
        foreach ($sp as $key => $value) {
            ...
            if (!in_array($key, WebSite::SEARCHABLE_FIELDS, true)) {
