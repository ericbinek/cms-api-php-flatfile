...
final class CategoryCodeSetRouter
{
    public const BASE = '/category-code-sets';
    private const ENTITY = 'CategoryCodeSet';
    ...
    private static function handleCollection(string $method, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'GET') {
            ...
            unset($opts['errors']);
            $all = CategoryCodeSet::findAll($opts);
        ...
        if ($method === 'POST') {
            ...
            $errors = CategoryCodeSet::validate($body);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $created = CategoryCodeSet::create(Access::applyCreateDefaults(self::ENTITY, $body, $principal['accountId']));
            Http::setLocation(self::BASE . '/' . $created['id']);
            Http::json(201, Access::stripFields($role, $created), etag: CategoryCodeSet::etagOf($created));
            return;
        }
    ...
    private static function handleItem(string $method, string $id, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'GET') {
            ...
            $item = CategoryCodeSet::findById($id);
            ...
            if ($item === null || !Access::isVisible($role, self::ENTITY, $item)) {
                Http::jsonError(Errors::notFound(CategoryCodeSet::TYPE_NAME, $requestPath));
                return;
            }
            // The ETag names the stored record's version, not the role- and
            // embedding-shaped body — it must satisfy a later If-Match.
            Http::json(200, Access::stripFields($role, CategoryCodeSet::embedRefs($item)), etag: CategoryCodeSet::etagOf($item));
            return;
        }
        ...
        if ($method === 'PUT') {
            ...
            $errors = CategoryCodeSet::validate($body, partial: true);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $current = CategoryCodeSet::findById($id);
            if ($current === null) {
                Http::jsonError(Errors::notFound(CategoryCodeSet::TYPE_NAME, $requestPath));
                return;
            }
            ...
            if ($ifMatch !== null && $ifMatch !== '*' && $ifMatch !== CategoryCodeSet::etagOf($current)) {
            ...
            $updated = CategoryCodeSet::update($id, $body);
            if ($updated === null) {
                Http::jsonError(Errors::notFound(CategoryCodeSet::TYPE_NAME, $requestPath));
                return;
            }
            Http::json(200, Access::stripFields($role, $updated), etag: CategoryCodeSet::etagOf($updated));
            return;
        }
    ...
    private static function parseListOptions(): array
    {
        ...
        $sort = 'dateCreated';
        if (isset($sp['sort'])) {
            if (!in_array($sp['sort'], CategoryCodeSet::SORTABLE_FIELDS, true)) {
                $sorted = CategoryCodeSet::SORTABLE_FIELDS;
                sort($sorted);
        ...
        foreach ($sp as $key => $value) {
            ...
            if (!in_array($key, CategoryCodeSet::SEARCHABLE_FIELDS, true)) {
