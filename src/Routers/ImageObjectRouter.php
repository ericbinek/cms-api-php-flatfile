...
final class ImageObjectRouter
{
    ...
    private static function handleCollection(string $method, string $requestPath): void
    {
        if ($method === 'GET') {
            ...
            unset($opts['errors']);
            Http::json(200, ImageObject::findAll($opts));
            return;
        }
        if ($method === 'POST') {
            $body = Http::parseBody();
            $errors = ImageObject::validate($body);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $created = ImageObject::create($body);
    ...
    private static function handleItem(string $method, string $id, string $requestPath): void
    {
        ...
        if ($method === 'GET') {
            $item = ImageObject::findById($id);
            if ($item === null) {
                Http::jsonError(Errors::notFound(ImageObject::TYPE_NAME, $requestPath));
                return;
            }
            Http::json(200, ImageObject::embedRefs($item));
            return;
        }

        if ($method === 'PUT') {
            $body = Http::parseBody();
            $errors = ImageObject::validate($body, partial: true);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $current = ImageObject::findById($id);
            if ($current === null) {
                Http::jsonError(Errors::notFound(ImageObject::TYPE_NAME, $requestPath));
                return;
            }
            $ifMatch = $_SERVER['HTTP_IF_MATCH'] ?? null;
            if ($ifMatch !== null && $ifMatch !== '*' && $ifMatch !== ImageObject::etagOf($current)) {
    ...
    private static function parseListOptions(): array
    {
        ...
        $sort = 'dateCreated';
        if (isset($sp['sort'])) {
            if (!in_array($sp['sort'], ImageObject::SORTABLE_FIELDS, true)) {
                $sorted = ImageObject::SORTABLE_FIELDS;
                sort($sorted);
        ...
        foreach ($sp as $key => $value) {
            ...
            if (!in_array($key, ImageObject::SEARCHABLE_FIELDS, true)) {
