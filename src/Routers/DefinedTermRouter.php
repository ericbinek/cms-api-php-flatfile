...
final class DefinedTermRouter
{
    public const BASE = '/defined-terms';
    private const ENTITY = 'DefinedTerm';
    ...
    private static function handleCollection(string $method, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'POST') {
            ...
            $body = DefinedTerm::sanitize(Http::parseBody());
            ...
            $errors = DefinedTerm::validate($body);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $created = DefinedTerm::create(Access::applyCreateDefaults(self::ENTITY, $body, $principal['accountId']));
            Http::setLocation(self::BASE . '/' . $created['id']);
            Http::json(201, Access::stripFields($role, $created), etag: DefinedTerm::etagOf($created));
            return;
        }
    ...
    private static function handleItem(string $method, string $id, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'GET') {
            ...
            Http::json(200, Access::stripFields($role, DefinedTerm::embedRefs($item)), etag: DefinedTerm::etagOf($item));
            return;
        }
        ...
        if ($method === 'PUT') {
            ...
            $errors = DefinedTerm::validate($body, partial: true);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $current = DefinedTerm::findById($id);
            if ($current === null) {
                Http::jsonError(Errors::notFound(DefinedTerm::TYPE_NAME, $requestPath));
                return;
            }
            ...
            if ($ifMatch !== null && $ifMatch !== '*' && $ifMatch !== DefinedTerm::etagOf($current)) {
    ...
    private static function parseListOptions(): array
    {
        ...
        $sort = 'dateCreated';
        if (isset($sp['sort'])) {
            if (!in_array($sp['sort'], DefinedTerm::SORTABLE_FIELDS, true)) {
                $sorted = DefinedTerm::SORTABLE_FIELDS;
                sort($sorted);
        ...
        foreach ($sp as $key => $value) {
            ...
            if (!in_array($key, DefinedTerm::SEARCHABLE_FIELDS, true)) {
