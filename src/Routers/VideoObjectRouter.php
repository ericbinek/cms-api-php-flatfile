...
final class VideoObjectRouter
{
    public const BASE = '/video-objects';
    private const ENTITY = 'VideoObject';
    ...
    private static function handleCollection(string $method, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'POST') {
            ...
            $body = VideoObject::sanitize(Http::parseBody());
            ...
            $errors = VideoObject::validate($body);
            if (!empty($errors)) {
    ...
    private static function handleItem(string $method, string $id, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'PUT') {
            ...
            $errors = VideoObject::validate($body, partial: true);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $current = VideoObject::findById($id);
            if ($current === null) {
                Http::jsonError(Errors::notFound(VideoObject::TYPE_NAME, $requestPath));
                return;
            }
            ...
            if ($ifMatch !== null && $ifMatch !== '*' && $ifMatch !== VideoObject::etagOf($current)) {
    ...
    private static function parseListOptions(): array
    {
        ...
        $sort = 'dateCreated';
        if (isset($sp['sort'])) {
            if (!in_array($sp['sort'], VideoObject::SORTABLE_FIELDS, true)) {
                $sorted = VideoObject::SORTABLE_FIELDS;
                sort($sorted);
        ...
        foreach ($sp as $key => $value) {
            ...
            if (!in_array($key, VideoObject::SEARCHABLE_FIELDS, true)) {
