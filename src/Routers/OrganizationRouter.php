...
final class OrganizationRouter
{
    ...
    private const ENTITY = 'Organization';
    ...
    private static function handleCollection(string $method, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'POST') {
            ...
            $body = Organization::sanitize(Http::parseBody());
            ...
            $errors = Organization::validate($body);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $created = Organization::create(Access::applyCreateDefaults(self::ENTITY, $body, $principal['accountId']));
            Http::setLocation(self::BASE . '/' . $created['id']);
            Http::json(201, Access::stripFields($role, $created), etag: Organization::etagOf($created));
            return;
        }
    ...
    private static function handleItem(string $method, string $id, string $requestPath, array $principal): void
    {
        ...
        if ($method === 'GET') {
            ...
            Http::json(200, Access::stripFields($role, Organization::embedRefs($item)), etag: Organization::etagOf($item));
            return;
        }
        ...
        if ($method === 'PUT') {
            ...
            $errors = Organization::validate($body, partial: true);
            if (!empty($errors)) {
                Http::jsonError(Errors::validation($errors, $requestPath));
                return;
            }
            $current = Organization::findById($id);
            if ($current === null) {
                Http::jsonError(Errors::notFound(Organization::TYPE_NAME, $requestPath));
                return;
            }
            ...
            if ($ifMatch !== null && $ifMatch !== '*' && $ifMatch !== Organization::etagOf($current)) {
    ...
    private static function parseListOptions(): array
    {
        ...
        $sort = 'dateCreated';
        if (isset($sp['sort'])) {
            if (!in_array($sp['sort'], Organization::SORTABLE_FIELDS, true)) {
                $sorted = Organization::SORTABLE_FIELDS;
                sort($sorted);
        ...
        foreach ($sp as $key => $value) {
            ...
            if (!in_array($key, Organization::SEARCHABLE_FIELDS, true)) {
