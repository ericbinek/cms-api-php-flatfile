...
test("$ENTITY: GET by id embeds \"image\" as an object; list stays flat", function () use ($ENTITY, $BASE) {
    ...
    cms_assert(is_string($inList['image'] ?? null), 'list ref should stay a UUID string');
});

test("$ENTITY: GET by id leaves an unresolvable \"image\" ref as its UUID", function () use ($ENTITY, $BASE) {
