...
test("$ENTITY: GET by id embeds \"inDefinedTermSet\" as an object; list stays flat", function () use ($ENTITY, $BASE) {
    ...
    $refId = $created['inDefinedTermSet'] ?? null;
    ...
    $embedded = $got['inDefinedTermSet'] ?? null;
    cms_assert(is_array($embedded), 'GET by id should embed the ref as an object');
    cms_assert_equal('DefinedTermSet', $embedded['@type'] ?? null);
    ...
    cms_assert(is_string($inList['inDefinedTermSet'] ?? null), 'list ref should stay a UUID string');
});

test("$ENTITY: GET by id leaves an unresolvable \"inDefinedTermSet\" ref as its UUID", function () use ($ENTITY, $BASE) {
    $dangling = '00000000-0000-0000-0000-000000000000';
    $payload = cms_build_payload($ENTITY, partial: true);
    $payload['inDefinedTermSet'] = $dangling;
    $created = cms_request('POST', $BASE, $payload)['body'];
    $got = cms_request('GET', "$BASE/{$created['id']}")['body'];
    cms_assert_equal($dangling, $got['inDefinedTermSet'] ?? null);
});
