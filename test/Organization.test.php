...
test("$ENTITY: GET by id embeds \"logo\" as an object; list stays flat", function () use ($ENTITY, $BASE) {
    ...
    cms_assert(is_string($inList['logo'] ?? null), 'list ref should stay a UUID string');
});

test("$ENTITY: GET by id leaves an unresolvable \"logo\" ref as its UUID", function () use ($ENTITY, $BASE) {
    $dangling = '00000000-0000-0000-0000-000000000000';
    $payload = cms_build_payload($ENTITY, partial: true);
    $payload['logo'] = $dangling;
    $created = cms_request('POST', $BASE, $payload)['body'];
    $got = cms_request('GET', "$BASE/{$created['id']}")['body'];
    cms_assert_equal($dangling, $got['logo'] ?? null);
});
