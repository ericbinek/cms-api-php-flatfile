...
test("$ENTITY: sort by headline accepted", function () use ($ENTITY, $BASE) {
    $r = cms_request('GET', "$BASE?sort=headline&order=asc");
    cms_assert_equal(200, $r['status']);
});
...
test("$ENTITY: sort by numeric field \"wordCount\" orders numerically with missing last", function () use ($ENTITY, $BASE) {
    $ids = [];
    ...
    $res = cms_request('GET', "$BASE?sort=wordCount&order=asc&limit=100");
    cms_assert_equal(200, $res['status']);
    $idSet = array_flip($ids);
    $mine = array_values(array_filter($res['body']['items'], static fn ($i) => isset($idSet[$i['id'] ?? ''])));
    $presentVals = [];
    foreach ($mine as $i) {
        if (isset($i['wordCount']) && (is_int($i['wordCount']) || is_float($i['wordCount']))) {
            $presentVals[] = $i['wordCount'];
        }
    }
...
test("$ENTITY: filter on text field \"headline\" returns matches", function () use ($ENTITY, $BASE) {
    $created = cms_request('POST', $BASE, cms_build_payload($ENTITY));
    $needle = substr((string) ($created['body']['headline'] ?? ''), 0, 4);
    if ($needle === '') return;
    $r = cms_request('GET', "$BASE?headline=" . rawurlencode($needle));
    $found = false;
...
test("$ENTITY: leading/trailing whitespace is trimmed on create", function () use ($ENTITY, $BASE) {
    ...
    cms_assert_equal('trimmed value', $r['body']['headline'] ?? null);
});
...
test("$ENTITY: value at maxLength accepted", function () use ($ENTITY, $BASE) {
    $payload = cms_build_payload($ENTITY);
    $payload['headline'] = str_repeat('a', 256);
    $r = cms_request('POST', $BASE, $payload);
    cms_assert_equal(201, $r['status'], 'expected 201: ' . $r['raw']);
});
...
test("$ENTITY: single-line field \"headline\" strips newlines", function () use ($ENTITY, $BASE) {
    $payload = cms_build_payload($ENTITY);
    $payload['headline'] = "first\nsecond";
    $r = cms_request('POST', $BASE, $payload);
    cms_assert_equal(201, $r['status'], 'expected 201: ' . $r['raw']);
    cms_assert_equal('firstsecond', $r['body']['headline'] ?? null);
});
...
test("$ENTITY: GET by id embeds \"image\" as an array of objects", function () use ($ENTITY, $BASE) {
    $payload = cms_build_payload($ENTITY, partial: true);
    $created = cms_request('POST', $BASE, $payload)['body'];
    $got = cms_request('GET', "$BASE/{$created['id']}")['body'];
    $embedded = $got['image'] ?? null;
    cms_assert(is_array($embedded) && array_is_list($embedded), 'embedded many-ref should be a list');
    cms_assert(count($embedded) >= 1, 'embedded many-ref should have at least one element');
    cms_assert(is_array($embedded[0]), 'embedded many-ref element should be an object');
    cms_assert_equal('ImageObject', $embedded[0]['@type'] ?? null);
});
