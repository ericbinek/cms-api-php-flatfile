...
test("$ENTITY: GET by id embeds \"inCodeSet\" as an object; list stays flat", function () use ($ENTITY, $BASE) {
    ...
    $embedded = $got['inCodeSet'] ?? null;
    cms_assert(is_array($embedded), 'GET by id should embed the ref as an object');
    cms_assert_equal('CategoryCodeSet', $embedded['@type'] ?? null);
    ...
    cms_assert(is_string($inList['inCodeSet'] ?? null), 'list ref should stay a UUID string');
});

test("$ENTITY: GET by id leaves an unresolvable \"inCodeSet\" ref as its UUID", function () use ($ENTITY, $BASE) {
